import about from "./about.jpg";
import agent from "./agent.png";
import amazon from "./amazon.png";
import arrowup from "./arrowup.png";
import avatar from "./avatar.png";
import background_pic from "./background-pic.png";
import ellipse_1 from "./ellipse-1.png";
import ellipse_2 from "./ellipse-2.png";
import google from "./google.png";
import house_1 from "./house_1.png";
import house_2 from "./house_2.png";
import house_3 from "./house_3.png";
import house_4 from "./house_4.png";
import house_5 from "./house_5.png";
import house_6 from "./house_6.png";
import house_7 from "./house_7.png";
import house_8 from "./house_8.png";
import house_9 from "./house_9.png";
import house_10 from "./house_10.png";
import house_11 from "./house_11.png";
import house_12 from "./house_12.png";
import house_13 from "./house_13.jpg";
import house_14 from "./house_14.png";
import house_15 from "./house_15.jpg";
import house_16 from "./house_16.png";
import hs_2 from "./hs_2.jpg";
import icon_arrow from "./icon_arrow.png";
import icon_right_arrow from "./icon_right_arrow.png";
import icon_left_arrow from "./icon_left_arrow.png";
import icon_bathroom from "./icon_bathroom.png";
import icon_bed from "./icon_bed.png";
import icon_blue_arrow from "./icon_blue_arrow.png";
import icon_facebook from "./icon_facebook.png";
import icon_fire from "./icon_fire.png";
import icon_hamburger from "./icon_hamburger.png";
import icon_home_blue from "./icon_home_blue.png";
import icon_instagram from "./icon_instagram.png";
import icon_linkedin from "./icon_linkedin.png";
import icon_setting from "./icon_setting.png";
import icon_smart_home from "./icon_smart_home.png";
import icon_user_circle from "./icon_user_circle.png";
import icon_user_octagon from "./icon_user_octagon.png";
import icon_search_white from "./icon_search_white.png";
import icon_home_white from "./icon_home_white.png";
import icon_bed_white from "./icon_bed_white.png";
import icon_phone from "./icon_phone.png";
import icon_mail from "./icon_mail.png";
import log from "./log.jpg";
import logitech from "./logitech.png";
import logo_blue from "./logo_blue.png";
import logo_white from "./logo_white.png";
import netflix from "./netflix.png";
import person_1 from "./person_1.png";
import person_2 from "./person_2.png";
import person_3 from "./person_3.png";
import person_4 from "./person_4.png";
import person_5 from "./person_5.png";
import person_6 from "./person_6.png";
import profile_pic from "./profile_pic.jpeg";
import quote from "./quote.png";
import rectangle_1 from "./rectangle_1.png";
import rectangle_2 from "./rectangle_2.png";
import rectangle_3 from "./rectangle_3.png";
import reg from "./reg.jpg";
import samsung from "./samsung.png";
import slide_1 from "./slide_1.jpg";
import slide_2 from "./slide_2.jpg";
import slide_3 from "./slide_3.jpg";
import slide_4 from "./slide_4.jpg";
import spotify from "./spotify.png";
import team_1 from "./team_1.png";
import team_2 from "./team_2.png";
import team_3 from "./team_3.png";
import team_4 from "./team_4.jpg";
import vector_dollar from "./vector_dollar.png";

export const assets = {
  about,
  agent,
  amazon,
  arrowup,
  avatar,
  background_pic,
  ellipse_1,
  ellipse_2,
  google,
  house_1,
  house_2,
  house_3,
  house_4,
  house_5,
  house_6,
  house_7,
  house_8,
  house_9,
  house_10,
  house_11,
  house_12,
  house_13,
  house_14,
  house_15,
  house_16,
  hs_2,
  icon_arrow,
  icon_right_arrow,
  icon_left_arrow,
  icon_bathroom,
  icon_bed,
  icon_blue_arrow,
  icon_facebook,
  icon_fire,
  icon_hamburger,
  icon_home_blue,
  icon_instagram,
  icon_linkedin,
  icon_setting,
  icon_smart_home,
  icon_user_circle,
  icon_user_octagon,
  icon_search_white,
  icon_home_white,
  icon_bed_white,
  icon_phone,
  icon_mail,
  log,
  logitech,
  logo_blue,
  logo_white,
  netflix,
  person_1,
  person_2,
  person_3,
  person_4,
  person_5,
  person_6,
  profile_pic,
  quote,
  rectangle_1,
  rectangle_2,
  rectangle_3,
  reg,
  samsung,
  slide_1,
  slide_2,
  slide_3,
  slide_4,
  spotify,
  team_1,
  team_2,
  team_3,
  team_4,
  vector_dollar,
};

export const properties = [
  {
    propertyName: "Luxury Duplex",
    propertyType: "House",
    purchaseType: "sale",
    status: "approved",
    address: "12 Ikoyi Crescent",
    city: "Lagos",
    state: "Lagos",
    price: 250000000,
    photos: [house_15, house_16, house_3],
    description: "A luxurious 5-bedroom duplex in the heart of Ikoyi.",
    bedroom: 5,
    bathroom: 6,
  },
  {
    propertyName: "Modern Apartment",
    propertyType: "Apartment",
    purchaseType: "rent",
    status: "pending",
    address: "23 Wuse 2 Avenue",
    city: "Abuja",
    state: "FCT",
    price: 3500000,
    photos: [house_11, house_16],
    description:
      "A modern 3-bedroom apartment with state-of-the-art facilities.",
    bedroom: 3,
    bathroom: 4,
  },
  {
    propertyName: "Ocean View Condo",
    propertyType: "Condo",
    purchaseType: "sale",
    status: "approved",
    address: "5 Admiralty Way",
    city: "Lekki",
    state: "Lagos",
    price: 60000000,
    photos: [house_1,house_2],
    description: "A stunning 2-bedroom condo with a breathtaking ocean view.",
    bedroom: 2,
    bathroom: 3,
  },
  {
    propertyName: "Affordable Bungalow",
    propertyType: "House",
    purchaseType: "sale",
    status: "pending",
    address: "34 GRA Road",
    city: "Benin",
    state: "Edo",
    price: 15000000,
    photos: [house_15, house_16, house_3],
    description: "A cozy 3-bedroom bungalow in a serene environment.",
    bedroom: 3,
    bathroom: 2,
  },
  {
    propertyName: "Executive Penthouse",
    propertyType: "Apartment",
    purchaseType: "rent",
    status: "approved",
    address: "18 Banana Island Drive",
    city: "Lagos",
    state: "Lagos",
    price: 10000000,
    photos: [house_3],
    description: "A luxurious penthouse in an exclusive neighborhood.",
    bedroom: 4,
    bathroom: 5,
  },
  {
    propertyName: "Cozy Condo",
    propertyType: "Condo",
    purchaseType: "sale",
    status: "rejected",
    address: "9 Enugu Crescent",
    city: "Enugu",
    state: "Enugu",
    price: 25000000,
    photos: [house_1],
    description: "A 2-bedroom condo with modern finishes.",
    bedroom: 2,
    bathroom: 2,
  },
  {
    propertyName: "Family Bungalow",
    propertyType: "House",
    purchaseType: "rent",
    status: "approved",
    address: "44 Ibadan Road",
    city: "Ibadan",
    state: "Oyo",
    price: 1200000,
    photos: [house_9],
    description: "A family-friendly 4-bedroom bungalow.",
    bedroom: 4,
    bathroom: 3,
  },
  {
    propertyName: "Stylish Loft",
    propertyType: "Apartment",
    purchaseType: "sale",
    status: "approved",
    address: "22 Victoria Island",
    city: "Lagos",
    state: "Lagos",
    price: 50000000,
    photos: [house_14],
    description: "A stylish loft apartment in a prime location.",
    bedroom: 2,
    bathroom: 2,
  },
  {
    propertyName: "Quiet Townhouse",
    propertyType: "Condo",
    purchaseType: "rent",
    status: "pending",
    address: "7 Asokoro Road",
    city: "Abuja",
    state: "FCT",
    price: 4000000,
    photos: [house_3],
    description: "A 3-bedroom townhouse in a quiet area.",
    bedroom: 3,
    bathroom: 3,
  },
  {
    propertyName: "Classic Detached House",
    propertyType: "House",
    purchaseType: "sale",
    status: "approved",
    address: "16 Independence Layout",
    city: "Enugu",
    state: "Enugu",
    price: 90000000,
    photos: [house_11],
    description: "A classic 6-bedroom detached house.",
    bedroom: 6,
    bathroom: 5,
  },
  {
    propertyName: "Modern Studio",
    propertyType: "Apartment",
    purchaseType: "rent",
    status: "pending",
    address: "3 Alausa Street",
    city: "Ikeja",
    state: "Lagos",
    price: 800000,
    photos: [house_15, house_16, house_3],
    description: "A modern studio apartment in a bustling area.",
    bedroom: 1,
    bathroom: 1,
  },
  {
    propertyName: "Spacious Duplex",
    propertyType: "House",
    purchaseType: "sale",
    status: "approved",
    address: "27 Aba Road",
    city: "Port Harcourt",
    state: "Rivers",
    price: 120000000,
    photos: [house_15, house_3],
    description: "A spacious 5-bedroom duplex in a prime location.",
    bedroom: 5,
    bathroom: 6,
  },
  {
    propertyName: "Luxury Beachfront Condo",
    propertyType: "Condo",
    purchaseType: "sale",
    status: "pending",
    address: "10 Oniru Crescent",
    city: "Lekki",
    state: "Lagos",
    price: 80000000,
    photos: [house_16],
    description: "A luxurious condo with direct access to the beach.",
    bedroom: 3,
    bathroom: 4,
  },
  {
    propertyName: "Elegant Villa",
    propertyType: "House",
    purchaseType: "sale",
    status: "approved",
    address: "15 Trans-Amadi Layout",
    city: "Port Harcourt",
    state: "Rivers",
    price: 200000000,
    photos: [house_15],
    description: "An elegant 6-bedroom villa with a swimming pool.",
    bedroom: 6,
    bathroom: 7,
  },
];
